...
if [ $# -lt 3 ]; then
  echo "Error: A workspace, workspace name and assembly must be provided"
  echo "Usage: $0 <workspace> <name> <assembly>"
  exit 1
fi
...
ssh -q -o "StrictHostKeyChecking=no" -o "UserKnownHostsFile=/dev/null" -i ssh_key.pem \
    "$ADMIN_USER"@"$public_ip" "sudo -u ghuser -g ghuser -i /bin/bash -c 'cd /opt/ghserver && ./ghadmin workspace-create \"${workspace}\" \"${name}\" --copy-config core --assembly \"${assembly}\"'"
